...
class AddressLibrary:
    ...
    def load_bin_v0(self, file_path):
        ...
        for i in range(count):
            off = 8 + i * 16
            iid, ioff = struct.unpack_from('<QQ', data, off)
            db[iid] = ioff
        return db

    def load_csv(self, file_path, skip_meta=False):
        if not os.path.exists(file_path):
            return {}
        db = {}
        ...
        if skip_meta and len(lines) > 1:
            start = 2  # skip metadata line '<count>,<format_version>'
            ...
            if s['t'] == 'label':
                ...
                if cu:
                    if final_name.startswith('RTTI_'):
                        cu.setComment(0, 'Source: CommonLibF4 (IDs_RTTI.h)')
                    elif final_name.startswith('VTABLE_'):
                        cu.setComment(0, 'Source: CommonLibF4 (IDs_VTABLE.h)')
                    elif s.get('src'):
